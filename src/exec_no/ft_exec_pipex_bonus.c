...
/*   Updated: 2024/06/07 17:17:17 by yohanafi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_proccess(t_simple_cmds *cmd, int nb, int argc, char **env_p)
{
	int	fd;
	int	sv_stdin;
...
static pid_t	ft_pipe(t_simple_cmds *cmd, char **env_p, int nb, int argc)
{
	pid_t	pid;
...
void	ft_multi_pipe(int argc, t_simple_cmds *list, char **env_p)
{
	int		i;
	int		status;
	pid_t	*pid;
	...
	while (++i <= argc)
	{
		pid[i] = ft_pipe(list, env_p, i, argc);
		if (list->next)
			list = list->next;
	}
	i = 0;
